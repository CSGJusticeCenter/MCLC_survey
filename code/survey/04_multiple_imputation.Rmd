...
#all flags should be 0 - if there is a 1 then stop program and check state and logic
stopifnot(data.frame(table(check.dat$flag_error.na))[,1] == 0) #flag new offense violation admissions logic
stopifnot(data.frame(table(check.dat$flag_error.np))[,1] == 0) #flag new offense violation population logic
stopifnot(data.frame(table(check.dat$flag_error.ta))[,1] == 0) #flag technical violation admissions logic
stopifnot(data.frame(table(check.dat$flag_error.tp))[,1] == 0) #flag technical violation population logic

#print states/years/metrics where zeroes exist; should exist only for Maine for Parole (Maine abolished parole)
cbind(state.zeroes,years.zeroes,var.zeroes)
#stop program if other states besides Maine have 0s in their data
stopifnot("Maine" %in% state.zeroes)
...
write.xlsx(specialmissings, file = paste0(sp_data_path, "/50 State Survey (2022)/Data/mclc_data_2022_v8_specialmissings.xlsx"))
