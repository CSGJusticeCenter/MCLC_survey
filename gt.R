...
df <- pop_table_checklist %>%
  ...
  select(-c(state))
  # select(-c(check_2018_21_22,check_2019_21_22, check_2020_21_22))
df <- tibble(df)
...
mytable <-  gt(df) %>%
  ...
  fmt_missing(columns = gt::everything(), missing_text = "No Data") %>%
  ...
  # change color to blue if there were updates to the data from 2018 - 2020, and new data for 2021
  tab_style(style = list(cell_fill(color = "#DEF0F6"),
  ...
  tab_style(style = list(cell_fill(color = "#DEF0F6"),
                         cell_text(weight = "bold")),
            locations = cells_body(columns = current_2021, rows = current_2021 != "No Data"))
